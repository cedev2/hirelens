...
const APPLICATION_FIELD_OPTIONS: { key: ApplicationFieldKey; label: string; description: string }[] = [
  { key: "cvUpload", label: "CV / Resume", description: "Upload resume file" },
  { key: "coverLetter", label: "Cover Letter", description: "Written cover letter" },
  { key: "skills", label: "Skills", description: "Technical & soft skills" },
  { key: "experience", label: "Work Experience", description: "Employment history" },
  { key: "education", label: "Education", description: "Academic background" },
  { key: "certifications", label: "Certifications", description: "Professional certificates" },
  { key: "phoneNumber", label: "Phone Number", description: "Contact phone" },
  { key: "location", label: "Location", description: "Applicant location" },
];
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {APPLICATION_FIELD_OPTIONS.map((fieldOption) => {
              const isChecked = watch("applicationFields")?.includes(fieldOption.key);
              return (
                <label
                  key={fieldOption.key}
                  className={`flex items-start gap-3 rounded-xl border p-3 cursor-pointer transition-all ${
                    isChecked
                      ? "border-[#087F5B] bg-white ring-1 ring-[#087F5B]/20"
                      : "border-gray-200 bg-white hover:border-gray-300"
                  }`}
                >
                  <input
                    type="checkbox"
                    className="mt-0.5 h-4 w-4 rounded border-gray-300 text-[#087F5B] focus:ring-[#087F5B]"
                    checked={isChecked}
                    onChange={(e) => {
                      const current = watch("applicationFields") || [];
                      if (e.target.checked) {
                        setValue("applicationFields", [...current, fieldOption.key], { shouldDirty: true });
                      } else {
                        setValue(
                          "applicationFields",
                          current.filter((f) => f !== fieldOption.key),
                          { shouldDirty: true }
                        );
                      }
                    }}
          ...
          {(!watch("applicationFields") || watch("applicationFields").length === 0) && (
            <p className="mt-3 text-xs font-semibold text-amber-700">
              Select at least one field for applicants to fill.
            </p>
          )}
        </div>
        ...
        >
          ...
          <div className="flex gap-2">
            ...
            <button
              type="submit"
              disabled={isSubmitting || createJobMutation.isPending}
              className="rounded-xl bg-[#087F5B] px-5 py-2 text-sm font-semibold text-white hover:bg-[#066B4D] disabled:opacity-50"
            >
